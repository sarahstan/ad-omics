...
class scDATA:
    def __init__(self, data_path, metadata_path, genes_path, barcodes_path):
        self.adata = None  # Will hold AnnData object
        self.metadata = None
        self.load_data(data_path, genes_path, barcodes_path)
        ...
        self.present_markers = [gene for genes in self.markers.values() 
                               for gene in genes if gene in self.adata.var_names]       
        print(f"Found {len(self.present_markers)} marker genes in dataset")
    
    def load_data(self, data_path, genes_path, barcodes_path):
        # Use scanpy to load Matrix Market files
        self.adata = sc.read_mtx(data_path)
        ...
        cells = pd.read_csv(barcodes_path, header=None)
        ...
        if self.adata.shape[1] == len(genes) and self.adata.shape[0] == len(cells):
            ...
            raise ValueError(f"With '{len(genes)}' genes and '{len(cells)}' cells, mismatch with Matrix shape: {self.adata.shape}") 
    ...
    def load_metadata(self, metadata_path, display=True):
        # Use pyreadr to load R metadata (.rds file)
        # example path for Windows: file_path = r'C:\Users\Sarah\Dropbox\Sharejerah\ROSMAP\data\ROSMAP.VascularCells.meta_full.rds'
        self.metadata = pyreadr.read_r(metadata_path)
    ...
    def run_pca(self, n_comps=30):
        ...
        if self.adata_hvg.n_obs < 3 or self.adata_hvg.n_vars < 3:
            print(f"Error: Not enough data for PCA. Found {self.adata_hvg.n_obs} cells and {self.adata_hvg.n_vars} genes.")
            return False
    ...
    def run_harmony(self, batch_key, max_iter_harmony=20, theta=2, lambda_val=1):
        ...
        try:
            ...
            print("Expected shape:", self.adata_hvg.shape[0], "x", self.adata_hvg.obsm['X_pca'].shape[1])
    ...
    def run_umap(self, use_harmony=True, n_neighbors=30, min_dist=0.3, metric='euclidean', n_components=2, random_state=42):
        """
        ...
        try:
            ...
            pca_key = 'X_pca_harmony' if use_harmony and 'X_pca_harmony' in self.adata_hvg.obsm else 'X_pca'
