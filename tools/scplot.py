...
class scPLOT:
    ...
    def plot_qc_metrics(self, save_path=None):
        ...
        # RNA counts
        fig.add_trace(
            go.Violin(y=self.sc_data.metadata['nCount_RNA'], box_visible=True, line_color='black',
                    fillcolor='lightseagreen', opacity=0.6, name='RNA counts'),
            row=1, col=1
        )
        # Number of genes
        fig.add_trace(
            go.Violin(y=self.sc_data.metadata['nFeature_RNA'], box_visible=True, line_color='black',
                    fillcolor='darkorange', opacity=0.6, name='Gene count'),
            row=1, col=2
        )
        # Mitochondrial percentage
        fig.add_trace(
            go.Violin(y=self.sc_data.metadata['percent.mt'], box_visible=True, line_color='black',
    ...
    def plot_qc_scatter(self,save_path=None):
        ...
        fig.add_trace(
            go.Scatter(x=self.sc_data.metadata['nFeature_RNA'], y=self.sc_data.metadata['nCount_RNA'],
        ...
        fig.add_trace(
            go.Scatter(x=self.sc_data.metadata['nFeature_RNA'], y=self.sc_data.metadata['percent.mt'],
    ...
    def plot_celltype_composition(self,cellcolumn,save_path=None):
        ...
        # Create pie chart
        fig = px.pie(celltype_counts, values='count', names=cellcolumn,
    ...
    def plot_preprocessing_state(self, save_path=None):
        ...
        fig.update_layout(
            height=800,
            width=1000,
            title_text="Preprocessing State Analysis",
            showlegend=True
        )
        # Update axes labels
    ...
    def plot_pca(self, color_by='celltype', save_path=None):
        ...
        pca_df = pd.DataFrame(self.sc_data.adata_hvg.obsm['X_pca_harmony'][:, :2], index=self.sc_data.adata_hvg.obs.index, columns=['PC1', 'PC2'])
        pca_df['celltype'] = self.sc_data.metadata.loc[pca_df.index, 'celltype']
        
        fig = px.scatter(
            pca_df, x='PC1', y='PC2', color='celltype',
    ...
    def plot_umap(self, color_by='celltype', save_path=None):
        ...
        self.sc_data.embedding_df[color_by] = self.sc_data.metadata.loc[self.sc_data.embedding_df.index, color_by]
                ...
                for j, ct in enumerate(expr_df[cell_type_col].unique()):
                    ...
                    fig.add_trace(
                        go.Violin(
                            ...
                            line_color=px.colors.qualitative.Bold[j % len(px.colors.qualitative.Bold)]
                        ),
                        row=1, col=i+1
                    )           
            fig.update_layout(
